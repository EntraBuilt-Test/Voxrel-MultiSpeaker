...
const OLD_MONGO_URI =
  'mongodb+srv://Samhitaops:Samhitaops123@cluster0.foslkww.mongodb.net/?appName=Cluster0';
const NEW_MONGO_URI =
...
async function migrateCollection(
  ...
  try {
    ...
    if (existingCount > 0) {
      console.log(
        `  ⚠️  Collection "${collectionName}" already has ${existingCount} documents in new DB`
      );
      const overwrite = process.argv.includes('--overwrite');
      if (!overwrite) {
        console.log(
          `  ⏭️  Skipping "${collectionName}" (use --overwrite to replace existing data)`
        );
        return stats;
      }
      ...
      try {
        ...
        if (i + batchSize < documents.length) {
          process.stdout.write(
            `    Progress: ${Math.min(i + batchSize, documents.length)}/${documents.length}\r`
          );
        }
      } catch (error: any) {
          ...
          for (const doc of batch) {
            try {
              ...
              if (err.code !== 11000) {
                stats.errors++;
                console.error(
                  `    ❌ Error inserting document in "${collectionName}":`,
                  err.message
                );
              }
            }
    ...
    console.log(
      `  ✅ Migrated ${stats.migrated}/${stats.total} documents from "${collectionName}"`
    );
    if (stats.errors > 0) {
  ...
  try {
    ...
    if (oldDbNames.length > 0) {
      ...
      for (const dbName of oldDbNames) {
        ...
        const hasUsers = collections.some((col: any) => col.name === 'users');
        if (hasUsers) {
    ...
    allStats.forEach(stat => {
      ...
      console.log(
        `  ${stat.collection.padEnd(20)} | Total: ${String(stat.total).padStart(6)} | ` +
    ...
    console.log('='.repeat(60));
    console.log(
      `  ${'TOTAL'.padEnd(20)} | Total: ${String(totalDocuments).padStart(6)} | ` +
...
function getDefaultDbName(uri: string): string {
