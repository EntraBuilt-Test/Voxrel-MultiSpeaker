...
class UserService extends BaseService {
  ...
  async login(credentials: LoginCredentials): Promise<{ user: User; accessToken: string; refreshToken: string }> {
    ...
    if (typeof window !== 'undefined') {
      ...
      // Tokens are stored, verification complete
  ...
  async registerAdmin(userData: RegisterData): Promise<ApiResponse<any>> {
    return this.post('/auth/register-admin', userData);
  }
  ...
  async logout(): Promise<void> {
    ...
    try {
      ...
      // Logout endpoint call failed, continue with local cleanup
    } finally {
  ...
  async getUserAnalyticsDashboard(dateFrom: string, dateTo: string, limit: number = 5): Promise<{
    success: boolean;
    statusCode: number;
    message: string;
    data: import('@/types').UserAnalyticsDashboard;
  }> {
    return this.get(`/admin/analytics/users/dashboard?dateFrom=${dateFrom}&dateTo=${dateTo}&limit=${limit}`);
  }

  // Individual Analytics Endpoints (use dashboard endpoint instead for better performance)
  async getUserAnalyticsSummary(dateFrom: string, dateTo: string): Promise<{
    success: boolean;
    statusCode: number;
    message: string;
    data: import('@/types').UserAnalytics;
  }> {
    return this.get(`/admin/analytics/users/summary?dateFrom=${dateFrom}&dateTo=${dateTo}`);
  }

  async getUserGrowthTrend(dateFrom: string, dateTo: string): Promise<{
    success: boolean;
    statusCode: number;
    message: string;
    data: import('@/types').UserGrowthTrend[];
  }> {
    return this.get(`/admin/analytics/users/growth-trend?dateFrom=${dateFrom}&dateTo=${dateTo}`);
  }

  async getTopPerformers(dateFrom: string, dateTo: string, limit: number = 5): Promise<{
    success: boolean;
    statusCode: number;
    message: string;
    data: import('@/types').TopPerformer[];
  }> {
    return this.get(`/admin/analytics/users/top-performers?dateFrom=${dateFrom}&dateTo=${dateTo}&limit=${limit}`);
  }
