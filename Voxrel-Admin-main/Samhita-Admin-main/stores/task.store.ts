...
interface TaskStore extends TaskStoreState {
  ...
  // --- GETTERS (SELECTORS) ---
  getTasksByStatus: (_status: string) => Task[];
  getTasksByPriority: (_priority: string) => Task[];
  getPendingTasksCount: () => number;
  getCompletedTasksCount: () => number;
  getOverdueTasksCount: () => number;
  getTaskById: (_id: string) => Task | undefined;

  // --- ACTIONS ---
  fetchTasks: (_page?: number, _limit?: number, _filters?: TaskFilters) => Promise<void>;
  fetchTaskById: (_id: string) => Promise<void>;
  createTask: (_taskData: CreateTaskData) => Promise<void>;
  updateTask: (_id: string, _data: UpdateTaskData) => Promise<void>;
  deleteTask: (_id: string) => Promise<void>;
  approveTaskClaim: (_id: string) => Promise<void>;
  rejectTaskClaim: (_id: string) => Promise<void>;
  clearCurrentTask: () => void;
  clearError: () => void;
  setFilters: (_filters: Partial<TaskFilters>) => void;
  setPagination: (_page: number, _limit?: number) => void;
}
