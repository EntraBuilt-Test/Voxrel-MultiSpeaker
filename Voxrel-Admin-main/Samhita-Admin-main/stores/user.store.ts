...
interface UserStore extends UserStoreState {
  ...
  register: (_userData: RegisterData) => Promise<void>;
  logout: () => Promise<void>;
  refreshToken: () => Promise<void>;
  getCurrentUser: () => Promise<void>;
  updateProfile: (_data: Partial<User>) => Promise<void>;
  changePassword: (_oldPassword: string, _newPassword: string) => Promise<void>;
  forgotPassword: (_email: string) => Promise<{ message: string; otp?: string; expiresIn?: string; }>;
  resetPassword: (_email: string, _otp: string, _newPassword: string) => Promise<{ message: string; }>;
  clearError: () => void;
  setUser: (_user: User | null) => void;
  setToken: (_token: string | null) => void;

  // --- ADMIN USER MANAGEMENT ACTIONS ---
  fetchUsers: (_page: number, _limit: number, _filters: UserFilters) => Promise<void>;
  createUser: (_userData: { name: string; email: string; password: string; role: 'ADMIN' | 'FREELANCER'; status?: 'ACTIVE' | 'PENDING_VERIFICATION' | 'BANNED' }) => Promise<User>;
  updateUser: (_userId: string, _data: Partial<User>) => Promise<void>;
...
const useUserStore = create<UserStore>()(
  persist(
    immer((set, get) => ({
      ...
      createUser: async (userData: { name: string; email: string; password: string; role: 'ADMIN' | 'FREELANCER'; status?: 'ACTIVE' | 'PENDING_VERIFICATION' | 'BANNED' }) => {
        set(state => {
          state.isLoading = true;
          state.error = null;
        });
