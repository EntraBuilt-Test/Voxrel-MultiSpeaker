...
import { TaskAnalytics, TaskCompletionTrend, TaskRevenueTrend } from '@/types';
...
export interface TaskAnalyticsState {
  ...
  // Utility functions
  formatTrend: (_value: number) => { arrow: string; color: string; displayText: string };
  ...
  // Handlers
  handleFilterChange: (_key: keyof TaskAnalyticsFilters, _value: string | undefined) => void;
  handleQuickFilterChange: (_value: string) => void;
  handleDateRangeChange: (_from: string, _to: string) => void;
...
export const useTaskAnalytics = (projectId?: string) => {
  ...
  const { dateFrom, dateTo } = useMemo(() => {
    ...
    // Format as YYYY-MM-DD for date-only queries (backend will handle timezone)
    const today = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
    
    if (typeof window !== 'undefined') {
      console.log('[Task Analytics] Calculating date range:', {
        quickFilter: filterValues.quickFilter,
        now: now.toISOString(),
        today
      });
    }
    ...
    switch (filterValues.quickFilter) {
      ...
      default: {
        ...
        if (typeof window !== 'undefined') {
          console.log('[Task Analytics] Default date range:', result);
        }
        return result;
      }
    }
  ...
  // Debug logging
  if (typeof window !== 'undefined') {
    console.log('[Task Analytics] Query States:', {
      dateFrom,
      dateTo,
      projectId,
      summaryLoading: analyticsSummaryQuery.isLoading,
      summaryError: analyticsSummaryQuery.error,
      summaryData: analyticsSummaryQuery.data,
      statusLoading: statusDistributionQuery.isLoading,
      statusData: statusDistributionQuery.data,
      completionLoading: completionTrendsQuery.isLoading,
      completionData: completionTrendsQuery.data,
    });
  }
  ...
  const statusDistribution = useMemo(() => {
    if (!statusDistributionQuery.data?.data) {
      if (typeof window !== 'undefined') {
        console.log('[Task Analytics] No status distribution data:', statusDistributionQuery.data);
      }
      return [];
    }

    const colors = ['#3b82f6', '#60a5fa', '#1d4ed8', '#93c5fd'];
    const processed = statusDistributionQuery.data.data.map((item, index) => ({
      ...item,
      fill: colors[index % colors.length]
    }));
    
    if (typeof window !== 'undefined') {
      console.log('[Task Analytics] Processed status distribution:', processed);
    }
  ...
  // Get analytics data with proper fallback
  const analyticsData = useMemo(() => {
    const data = analyticsSummaryQuery.data?.data;
    if (typeof window !== 'undefined') {
      console.log('[Task Analytics] Raw analytics data:', {
        rawResponse: analyticsSummaryQuery.data,
        extractedData: data,
        usingDefault: !data
      });
    }
