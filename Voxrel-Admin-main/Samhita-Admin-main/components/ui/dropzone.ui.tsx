...
export function Dropzone({
    ...
    const handleFiles = (newFiles: FileList | null) => {
        ...
        // Validate files based on accept prop
        let validFiles = fileArray;
        if (accept) {
            // Parse accept string (e.g., "audio/*", ".pdf,.doc,.docx", "application/pdf,application/msword")
            ...
            validFiles = fileArray.filter(file => {
                ...
                return acceptTypes.some(type => type.toLowerCase() === fileExt);
            });
        }
                        ...
                        {isDragOver ? (
                            ...
                            <>
                                <p className="font-medium mb-1">
                                    {maxFiles === 1 ? 'Upload File' : `Upload Files (max ${maxFiles})`}
                                </p>
                                <p className="text-muted-foreground mb-2">
                                    {accept === 'audio/*'
                                        ? (maxFiles === 1 ? 'Required • Supports MP3, WAV, M4A, OGG' : 'Drag & drop multiple files or click to browse • Supports MP3, WAV, M4A, OGG')
                                        : (maxFiles === 1 ? 'Required • Supports PDF, DOC, DOCX, TXT' : 'Drag & drop multiple files or click to browse • Supports PDF, DOC, DOCX, TXT')
                                    }
                                </p>
                                <button
                                    type="button"
                                    className="px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm font-medium hover:bg-primary/90 transition-colors pointer-events-auto"
                                    onClick={handleClick}
                                    disabled={disabled}
                                >
