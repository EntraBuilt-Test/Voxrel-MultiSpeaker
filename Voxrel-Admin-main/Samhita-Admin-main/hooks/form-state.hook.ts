...
interface UseFormStateReturn<T> {
  ...
  isValid: boolean;
  isDirty: boolean;
  handleChange: (_field: keyof T) => (_e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void;
  handleSelectChange: (_field: keyof T) => (_value: string) => void;
  handleDateChange: (_field: keyof T) => (_date: Date | undefined) => void;
  handleSubmit: (_onSubmit: (_values: T) => void | Promise<void>) => (_e?: React.FormEvent) => void;
  setValue: <K extends keyof T>(_field: K, _value: T[K]) => void;
  setValues: (_values: Partial<T>) => void;
  validateField: (_field: keyof T) => void;
  validateAll: () => boolean;
  reset: (_newValues?: T) => void;
  clearErrors: () => void;
}
...
export function useFormState<T extends Record<string, any>>(
  ...
  const handleSubmit = useCallback((onSubmit: (_values: T) => void | Promise<void>) => {
